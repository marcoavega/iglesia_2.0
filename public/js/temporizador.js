...
function a() {
    x = setInterval(function() {
        ...
        if (remaining < 0) {
            clearInterval(x);
            document.getElementById("display").innerHTML = "Tiempo terminado";
        }
        ...
        if (right < 90) {
            line = ['linear-gradient(' + right + 'deg,  rgb( 102, 255, 255) 50%, transparent 50%)',
                'linear-gradient(-90deg, #d11904 50%, transparent 50%)'
            ];
        }

        //rotates the blue, shows blue on both sides
        else {
            line = ['linear-gradient(' + left + 'deg, #d11904 50%, transparent 50%)',
                'linear-gradient(-90deg, #d11904 50%, transparent 50%)'
            ];
        }
...
$('#reset').on('click', function() {
    clearInterval(x);
    slice = $('#timer').css({
        'background-image': 'linear-gradient(-90deg,  rgb( 102, 255, 255) 50%, transparent 50%)'
    });
    tf = 7;
    display();
    pauseTime = NaN;
